...
let Cadastros = [{Login: 'test', Password: 'test', Name: 'Lucas Mendonça'}];
...
$('#botao-entrar').click(function (e) {
  ...
  if ($('#senha-login').val() && $('#email-login').val()) {
    ...
    if (!$('#senha-login').val()) { $('#senha-login').css('border-bottom', '1px solid red') }
    if (!$('#email-login').val()) { $('#email-login').css('border-bottom', '1px solid red') }
...
$('#botao-cadastrar').click(function (e) {
  ...
  if ($('#nome-cadastro').val() && $('#email-cadastro').val() && $('#senha-cadastro').val() && $('#senha-cadastro-confirm').val()) {
    ...
    //os campos ficam com a borda de baixo vermelha caso nao estejam preenchidos
    $('.error-text').show().text('Preencha todos os campos');
    if (!$('#nome-cadastro').val()) { $('#nome-cadastro').css('border-bottom', '1px solid red') }
    if (!$('#email-cadastro').val()) { $('#email-cadastro').css('border-bottom', '1px solid red') }
    if (!$('#senha-cadastro').val()) { $('#senha-cadastro').css('border-bottom', '1px solid red') }
    if (!$('#senha-cadastro-confirm').val()) { $('#senha-cadastro-confirm').css('border-bottom', '1px solid red') }
  }
});
